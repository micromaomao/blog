...
The gdb backtrace here doesn't tell us the full picture (although it does tell us that there are functions above [`handle_mm_fault`](https://github.com/micromaomao/linux-dev/blob/v6.12.1/mm/memory.c#L6042) which weren't traced). In fact, the kernel's own `bt` does a better job when things cross an exception or interrupt boundary (which in this case would be the page fault):
...
There is another case, relevant to us, where the kernel will make a page writable for the user following a write fault. This is when an anonymous page is first written to. (A first _read_ of such a page doesn't allocate anything &ndash; the kernel just maps in the shared, read-only zero page, and it's the subsequent write that faults again and gets us here.) This is due to memory overcommitment, in which pages are not actually &lsquo;allocated&rsquo; until the user program tries to use it. In this case, we will hit this `if` branch in [`handle_pte_fault`](https://github.com/micromaomao/linux-dev/blob/5996393469d99560b7845d22c9eff00661de0724/mm/memory.c#L5732), rather than going to [`do_wp_page`](https://github.com/micromaomao/linux-dev/blob/v6.12.1/mm/memory.c#L3655):
...
And for an anonymous mappings, the kernel will allocate a new zero-filled page on write. We therefore need our own logic there to also track newly allocated pages, and either deallocate them or revert them to be zero-filled on reverting the checkpoint.
...
For me, trying to implement something like this, play with kgdb, etc. is a very good way to learn and challenge my understanding, and part of that learning is also writing this article.  I hope that this has been a fun journey for you, and if you weren't already familiar with the Linux kernel, I hope that this has been an enjoyable introduction, much like that Orange book is to me on OS development.
